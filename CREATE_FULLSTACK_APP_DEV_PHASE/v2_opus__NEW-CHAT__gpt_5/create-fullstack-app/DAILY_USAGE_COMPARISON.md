# Daily Usage Comparison NEW

## Before
- Manual Node/Java installs on server
- Hand-built frontend, PM2 restarts
- JAR swaps, ad-hoc logs
- SSL via manual Certbot

## After
- Frontend and backend run in isolated containers
- Reproducible builds on Mac and server
- Single `docker compose up -d` to run
- Easy rollbacks with `git revert` + rebuild
