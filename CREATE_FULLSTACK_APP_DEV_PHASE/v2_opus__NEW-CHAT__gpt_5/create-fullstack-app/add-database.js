...
async function addDatabase() {
  ...
  if (!(await fs.pathExists("docker-compose.yml"))) {
    console.error(
      chalk.red(
        "❌ No docker-compose.yml found. Are you in a fullstack app directory?",
      ),
    );
    process.exit(1);
  }
  ...
  const answers = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        {
          name: "External PostgreSQL (Neon/Supabase)",
...
async function updateBackendConfig(dbType, credentials) {
  ...
  if (!(await fs.pathExists(backendPropsPath))) {
    console.log(
      chalk.yellow("⚠️  Backend application.properties not found, skipping..."),
    );
    return;
  }
